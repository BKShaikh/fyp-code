...
from skimage.color import rgb2gray  #additon
from skimage.exposure import adjust_gamma
import scipy.ndimage as ndimage  #addition
from scipy import misc  #additon
from skimage.segmentation import mark_boundaries
from sklearn.feature_extraction.image import extract_patches_2d
from sklearn.metrics import classification_report
from keras.models import Sequential, model_from_json
# from keras.models import Graph, Merge, MaxoutDense
...
import os
import os.path #additional
import matplotlib.image as mpimg  #addition from my side
import progressbar  #addition
# from tensorflow.python.ops import control_flow_ops
# tf.python.control_flow_ops = tf
# tensorflow.python.control_flow_ops = control_flow_ops
progress = progressbar.ProgressBar(widgets=[progressbar.Bar('*', '[', ']'), progressbar.Percentage(), ' ']) #addition
class SegmentationModel(object):
    def __init__(self, n_epoch=10, n_chan=4, batch_size=128, loaded_model=False, architecture='single', w_reg=0.01, n_filters=[64,128,128,128], k_dims = [7,5,5,3], activation = 'relu'):
        ...
        if not self.loaded_model:
            ...
            model = str(input('Which model should I load? '))   #raw_input = input
    ...
    def load_model_weights(self, model_name):
        ...
        with open(model) as f:
            m = f.readline()        #using readline instead of next
        model_comp = model_from_json(json.loads(m))
        model_comp.load_weights(weights)
        model_comp = self.compile_model()
        print ('Done.123')
        return model_comp
    ...
    def fit_model(self, X_train, y_train, X5_train = None, save=True):
        ...
        '''
        print('idher aya' , ' aur aya ', y_train)
        Y_train = np_utils.to_categorical(y_train, 5)

        shuffle = list(zip(X_train, Y_train))  #list() for python 3 as it needs to be converted to list first
        ...
        checkpointer = ModelCheckpoint(filepath="models/bm_{epoch:02d}-{val_loss:.2f}.hdf5", verbose=1)
    ...
    def predict_image(self, test_img):
        ...
        full_pred = self.model_comp.predict_classes(patches_to_predict)
        fp1 = full_pred.reshape(184, 128)
        return fp1

    def save_segmented_image(self,index ,test_img, modality='t1c', show = False): #save_segmented_image tha
        '''
        ...
        sigma=0
        print(test_img)
        # model=self.model_comp
        segmentation = self.predict_image(test_img)#, show=False)      #false tha
        ...
        test_back = test_im.reshape(5,216,160)[-2]
        ...
        if show:
            try:
                # mkdir_p('./results/')
                io.imsave('models/results/result' + '_' + str(
                        index) + '.png', sliced_image)
            except:
                io.imsave('models/results/result' + '_' + str(
                        index) + '.png', sliced_image)
    ...
    def get_dice_coef(self, test_img, label):
        ...
        INPUT   (1) str 'test_img': filepat test_img
        OUTPUT: Summary of dice scores for the following classes:
                    - all classesh to slice to predict on
                (2) str 'label': filepath to ground truth label for
        ...
        '''
        # test_img likha tha line 326 main aur label in 328 main
        segmentation = self.predict_image('Norm_PNG/**.png')
        seg_full = np.pad(segmentation, (16,16), mode='edge')
        gt = io.imread('Training/HGG/VSD.Brain_3more.XX.O.OT.54517/**.mha').astype(int)
...
if __name__ == '__main__':
    ...
    patches = PatchLibrary((33, 33), train_data, 10, 0)   #33,33 -> 26,33
    ...
    print(X,y,'aaa')
    model.fit_model(X, y)
    model.save_model('models/example')  #model.save_model('models/example')

    # tests = glob('test_data/**.**')
    # # tests = glob('test_data/2_*')
    # print(tests);
    # # test_sort = sorted(tests, key= lambda x:int(x[3:-4]))
    # model = SegmentationModel(loaded_model=True)
    # segmented_images = []
    # # print(segmented_images)
    # for index,slice in enumerate(tests):                    #test_sort[15:145]:
    #     # print(enumerate(tests))
    #     # segmented_images.append(model.save_segmented_image(slice))
    #     segmented_images.append(model.save_segmented_image(index,test_img=slice))
